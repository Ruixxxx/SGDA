...
def extract_main(binary_mask, cover=0.95):
    ...
    binary_mask: 3D binary numpy array with the same shape of the image,
        ...
        specific case.
    cover: float, percentage of the total area to keep of each slice, by
        keeping the total connected components
    return: binary mask with the same shape of the image, that only region of
        interest is True. One side of the lung in this specific case.
    """
...
def apply_mask(image, binary_mask1, binary_mask2, pad_value=170,
    ...
    pad_value: int, uint8 value for padding image regions that is not
        interested.
    bone_thred: int, uint8 threshold value for determine parts of image is
        bone.
...
def get_lung_box(pid, binary_mask, new_shape, margin=5):
    """
    ...
    new_shape: tuple of int * 3, new shape of the image after resamping in
        [z, y, x] order.
    margin: int, number of voxels to extend the boundary of the lung box.
    ...
    if z_true.shape[0] == 0 or y_true.shape[0] == 0 or x_true.shape[0] == 0:
        print(pid)
        exit()
...
def preprocess(params):
    pid, lung_mask_dir, img_dir, save_dir, do_resample, do_unit8 = params
    pid = pid.replace('.nii.gz', '')
    print('Preprocessing %s...' % (pid))

    lung_mask, _, _ = load_itk_image(os.path.join(lung_mask_dir, '%s.nii.gz' % (pid)))
    img, origin, spacing = load_itk_image(os.path.join(img_dir, '%s.nii.gz' % (pid)))
    ...
    lung_box = get_lung_box(pid, binary_mask, seg_img.shape)
    ...
    # if do_resample:
    #     np.save(os.path.join(save_dir, '%s_resampled_spacing.npy' % (pid)), resampled_spacing)
    np.save(os.path.join(save_dir, '%s_lungbox_origin.npy' % (pid)), np.array((z_min, y_min, x_min)))
    np.save(os.path.join(save_dir, '%s_clean.npy' % (pid)), seg_img)
...
def main():
    ...
    # datasets_list = ['pn9/train', 'pn9/test']
    for dataset in datasets_list:
        ...
        #     do_resample = False
        #     do_unit8 = False
        lung_mask_dir = '/home/xurui/data0/{}/masks/'.format(dataset)
        # if 'pn9' in dataset:
        #     lung_mask_dir = '/data0/{}/newmasks/'.format(dataset)
        img_dir = '/home/xurui/data0/{}/images/'.format(dataset)
        # if 'pn9' in dataset:
        #     img_dir = '/data0/{}/newimages/'.format(dataset)
        save_dir = '/home/xurui/data0/{}/preprocessed/'.format(dataset)
        ...
        params_lists = []
        # path_lists = os.listdir(img_dir)
        # path_lists.sort()
        for pid in os.listdir(img_dir):
            params_lists.append([pid, lung_mask_dir, img_dir, save_dir, do_resample, do_unit8])
    ...
    # pid = '1.3.6.1.4.1.14519.5.2.1.6279.6001.257515388956260258681136624817.nii.gz'
    # do_resample = True
    # do_unit8 = True
    # lung_mask_dir = '/home/xurui/data0/luna16/masks/'
    # img_dir = '/home/xurui/data0/luna16/images/'
    # save_dir = '/home/xurui/data0/luna16/preprocessed/'
    # if not os.path.exists(save_dir):
    #     os.makedirs(save_dir)
    #
    # params_lists = [pid, lung_mask_dir, img_dir, save_dir, do_resample, do_unit8]
