...
import numpy as np
import torch
from net.uni_layer.rpn_nms import rpn_encode
from torch.autograd import Variable


def make_one_rpn_target(cfg, mode, input, window, truth_box, truth_label, cls_ind):
    """
    ...
    truth_label: list of ground truth class label for each object in the corresponding truth_box
    ...
    if num_truth_box:
        ...
        # a threshold
        bg_index = np.where(max_overlap < cfg['rpn_train_bg_thresh_high'][cls_ind])[0]
        label[bg_index] = 0
        label_weight[bg_index] = 1

        # The anchor box is a sure foreground, if its largest IoU is larger or 
        # equal than a threshold
        fg_index = np.where(max_overlap >= cfg['rpn_train_fg_thresh_low'][cls_ind])[0]
        label[fg_index] = 1
...
def make_rpn_target(cfg, mode, inputs, window, truth_boxes, truth_labels, cls_ind):
    rpn_labels = []
    rpn_label_assigns = []
    rpn_label_weights = []
    rpn_targets = []
    ...
    for b in range(batch_size):
        ...
        rpn_label, rpn_label_assign, rpn_label_weight, rpn_target, rpn_targets_weight = \
            make_one_rpn_target(cfg, mode, input, window, truth_box, truth_label, cls_ind)
