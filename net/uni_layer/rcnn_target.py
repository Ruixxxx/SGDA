...
def make_one_rcnn_target(cfg, input, proposal, truth_box, truth_label, cls_ind):
    ...
    if len(truth_box) == 0:
        num_bg = min(len(proposal), cfg['rcnn_train_batch_size'][cls_ind])
    ...
    fg_index = np.where(max_overlap >= cfg['rcnn_train_fg_thresh_low'][cls_ind])[0]
    bg_index = np.where(max_overlap <  cfg['rcnn_train_bg_thresh_high'][cls_ind])[0]

    # sampling for class balance
    num_class = cfg['num_class'][cls_ind]
    num = cfg['rcnn_train_batch_size'][cls_ind]
    num_fg = int(np.round(cfg['rcnn_train_fg_fraction'][cls_ind] * cfg['rcnn_train_batch_size'][cls_ind]))
...
def make_rcnn_target(cfg, mode, inputs, proposals, truth_boxes, truth_labels, cls_ind):
    ...
    for b in range(batch_size):
        ...
        sampled_proposal, sampled_label, sampled_assign, sampled_target = \
           make_one_rcnn_target(cfg, input, proposal, truth_box, truth_label, cls_ind)
        # if sampled_assign.shape[0] != 64:
        #     print('bad data:', filename[b])
        #     print(sampled_assign.shape)
