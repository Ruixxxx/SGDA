...
def rcnn_nms(cfg, mode, inputs, proposals, logits, deltas, cls_ind):

    if mode in ['train',]:
        nms_pre_score_threshold = cfg['rcnn_train_nms_pre_score_threshold'][cls_ind]
        nms_overlap_threshold   = cfg['rcnn_train_nms_overlap_threshold'][cls_ind]

    elif mode in ['valid', 'test','eval']:
        nms_pre_score_threshold = cfg['rcnn_test_nms_pre_score_threshold'][cls_ind]
        nms_overlap_threshold   = cfg['rcnn_test_nms_overlap_threshold'][cls_ind]
        #
        # if mode in ['eval']:
...
def get_probability(cfg, mode, inputs, proposals, logits, deltas, cls_ind):
    if mode in ['train',]:
        nms_pre_score_threshold = cfg['rcnn_train_nms_pre_score_threshold'][cls_ind]
        nms_overlap_threshold   = cfg['rcnn_train_nms_overlap_threshold'][cls_ind]
