...
from net.sgda_sanet import da_resnet_wt_v2droppath_middle_top
...
class DAResBlock3d(nn.Module):
    def __init__(self, n_in, n_out, net_mode, drop_path, stride=1, fixed_block=False, reduction=16):
        ...
        self.domain_attention = DomainAttention(n_out, reduction=16, nclass_list=config['num_class'],
...
class DAFeatureNet(nn.Module):
    def __init__(self, config, net_mode):
        super(DAFeatureNet, self).__init__()
        self.resnet50 = da_resnet_wt_v2droppath_middle_top.da_resnet50(net_mode=net_mode)
...
class SGDASANet_wt_v2droppath_middle_top(nn.Module):
    def __init__(self, cfg, mode='train'):
        super(SGDASANet_wt_v2droppath_middle_top, self).__init__()
        self.cfg = cfg
        self.mode = mode
        self.feature_net = DAFeatureNet(config, self.mode)
...
if __name__ == '__main__':
    net = SGDASANet_wt_v2droppath_bottom_top(config)
