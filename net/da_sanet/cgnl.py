...
#         x = self.z(x)
#         # 固定深度方向
#         num_gn = int(self.groups / 4)
#         if num_gn and num_gn > 1:
#             _c = int(zs / num_gn)
#             xs = torch.split(x, split_size_or_sections=_c, dim=2)
#             _x_sequences = []
#             for i in range(num_gn):
#                 _x = self.gn(xs[i])
#                 _x_sequences.append(_x)
#             x = torch.cat(_x_sequences, dim=2)
#             x = x + residual
#         else:
#             x = self.gn(x) + residual
#
