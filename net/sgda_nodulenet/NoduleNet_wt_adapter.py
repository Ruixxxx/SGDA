...
from configs.config_wt_adapter import config
import copy
...
# def norm_act(in_channels, only='both'):
#     if config.instance_norm:
#         norm = nn.InstanceNorm3d(in_channels, affine=True)
...
def norm_act(in_channels, only='both'):
    norm = nn.BatchNorm3d(in_channels, momentum=bn_momentum)
    act = nn.ReLU(inplace=True) # activation
    if only=='norm':
        return norm
    elif only=='act':
        return act
...
class conv1x1(nn.Module):
    def __init__(self, in_channels, out_channels=None, stride=1, padding=0):
        super(conv1x1, self).__init__()
        if config['module'] == 'series_adapter':
            self.op1 = nn.Sequential(
                norm_act(in_channels,only='norm'),
                nn.Conv3d(in_channels, in_channels, kernel_size=1, stride=1)
                )
        elif config['module'] == 'parallel_adapter':
            self.op1 = nn.Conv3d(in_channels, out_channels, kernel_size=1, stride=stride, padding=padding)
        else:
            self.op1 = nn.Conv3d(in_channels, in_channels, kernel_size=1, stride=1)
...
class dwise(nn.Module):
    def __init__(self, in_channels, kernel_size=3, stride=1, padding=1):
        ...
        self.conv1 = nn.Conv3d(in_channels, in_channels, kernel_size=kernel_size, stride=stride, padding=padding, groups=in_channels)
        self.op1 = norm_act(in_channels, only='both')
...
class conv_unit(nn.Module):
    ...
    '''
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, second=0):
        super(conv_unit, self).__init__()
        self.stride = stride

        if self.stride != 1:
            self.conv = nn.Conv3d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding) # padding != 0 for stride != 2 if doing padding=SAME.
        elif self.stride == 1:
            if config['trainMode'] != 'universal': # independent, shared
                self.conv = nn.Conv3d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding) # padding != 0 for stride != 2 if doing padding=SAME.
            else:
                if config['module'] in ['series_adapter', 'parallel_adapter', 'BN_adapter']:
                    self.conv = nn.Conv3d(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding) # padding != 0 for stride != 2 if doing padding=SAME.
                    if config['module'] == 'series_adapter':
                        self.adapOps = nn.ModuleList([conv1x1(out_channels) for i in range(config['num_datasets'])]) # based on https://github.com/srebuffi/residual_adapters/
                    elif config['module'] == 'parallel_adapter':
                        self.adapOps = nn.ModuleList([conv1x1(in_channels, out_channels) for i in range(config['num_datasets'])])
                    else:
                        pass
                elif config['module'] == 'separable_adapter':
                    # logger.info('using module of :{}'.format(config['module']))
                    self.adapOps = nn.ModuleList([dwise(in_channels) for i in range(config['num_datasets'])])
                    self.pwise = pwise(in_channels, out_channels)
                else:
                    pass                

        self.op = nn.ModuleList([norm_act(out_channels, only='norm') for i in range(config['num_datasets'])])

    def forward(self, x, cls_ind):
        if self.stride != 1:
            out = self.conv(x)
            out = self.op[cls_ind](out)
            return out
        elif self.stride == 1:
            if config['trainMode'] != 'universal': # independent, shared
                out = self.conv(x)
                ...
                if config['module'] in ['series_adapter', 'parallel_adapter', 'BN_adapter']:
                    out = self.conv(x)
                    if config['module'] == 'series_adapter':
                        out = self.adapOps[cls_ind](out)
                    elif config['module'] == 'parallel_adapter':
                        share_map = out
                        para_map = self.adapOps[cls_ind](x)
                        out = out + para_map
                    else:
                        pass
...
class ResBlock3d_wt_adapter(nn.Module):
    def __init__(self, n_in, n_out, stride=1):
        super(ResBlock3d_wt_adapter, self).__init__()
        self.conv1 = conv_unit(n_in, n_out, kernel_size=3, stride=stride, padding=1)
        self.relu = nn.ReLU(inplace = True)
        self.conv2 = conv_unit(n_out, n_out, kernel_size=3, padding=1)

        if stride != 1 or n_out != n_in:
            self.shortcut = conv_unit(n_in, n_out, kernel_size=1, stride=stride, padding=0)
        else:
            self.shortcut = None
    ...
    def forward(self, x, cls_ind):
        ...
        if self.shortcut is not None:
            if config['module'] == 'parallel_adapter' or config['module'] == 'separable_adapter':
                residual, share_map, para_map = self.shortcut(x, cls_ind)
            else:
                residual = self.shortcut(x, cls_ind)
        if config['module'] == 'parallel_adapter' or config['module'] == 'separable_adapter':
            out, share_map, para_map = self.conv1(x, cls_ind)
        else:
            out = self.conv1(x, cls_ind)
        out = self.relu(out)
        if config['module'] == 'parallel_adapter' or config['module'] == 'separable_adapter':
            out, share_map, para_map = self.conv2(out, cls_ind)
        else:
            out = self.conv2(out, cls_ind)
...
class FeatureNet_wt_adapter(nn.Module):
    def __init__(self, config, in_channels, out_channels):
        super(FeatureNet_wt_adapter, self).__init__()
        ...
        self.forw1_1 = ResBlock3d_wt_adapter(24, 32)
        self.forw1_2 = ResBlock3d_wt_adapter(32, 32)

        self.forw2_1 = ResBlock3d_wt_adapter(32, 64)
        self.forw2_2 = ResBlock3d_wt_adapter(64, 64)

        self.forw3_1 = ResBlock3d_wt_adapter(64, 64)
        self.forw3_2 = ResBlock3d_wt_adapter(64, 64)
        self.forw3_3 = ResBlock3d_wt_adapter(64, 64)

        self.forw4_1 = ResBlock3d_wt_adapter(64, 64)
        self.forw4_2 = ResBlock3d_wt_adapter(64, 64)
        self.forw4_3 = ResBlock3d_wt_adapter(64, 64)

        # skip connection in U-net
        # 64 + 64 + 3, where 3 is the channeld dimension of coord
        self.back2_1 = ResBlock3d_wt_adapter(128, 128)
        self.back2_2 = ResBlock3d_wt_adapter(128, 128)
        self.back2_3 = ResBlock3d_wt_adapter(128, 128)

        # skip connection in U-net
        self.back3_1 = ResBlock3d_wt_adapter(128, 64)
        self.back3_2 = ResBlock3d_wt_adapter(64, 64)
        self.back3_3 = ResBlock3d_wt_adapter(64, 64)
...
class NoduleNet_wt_adapter(nn.Module):
    def __init__(self, cfg, mode='train'):
        super(NoduleNet_wt_adapter, self).__init__()
        self.cfg = cfg
        self.mode = mode
        self.feature_net = FeatureNet_wt_adapter(config, 1, 128)
...
if __name__ == '__main__':
    net = NoduleNet_wt_adapter(config)
