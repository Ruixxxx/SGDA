...
from net.sgda.domain_attention_module3 import DomainAttention
...
class DAResBlock3d(nn.Module):
    def __init__(self, n_in, n_out, stride=1, fixed_block=False, reduction=16):
        ...
        self.domain_attention = DomainAttention(n_out, reduction=16, nclass_list=cfg['num_class'], fixed_block=fixed_block,
                                                groups=config['groups'], mode='noG_1D', attention_dir=True)
...
class DAFeatureNet(nn.Module):
  def __init__(self, config, in_channels, out_channels):
    ...
    fixed_block = False
    self.forw1_1 = DAResBlock3d(24, 32, fixed_block=fixed_block)
    self.forw1_2 = DAResBlock3d(32, 32, fixed_block=fixed_block)

    self.forw2_1 = DAResBlock3d(32, 64, fixed_block=fixed_block)
    self.forw2_2 = DAResBlock3d(64, 64, fixed_block=fixed_block)

    self.forw3_1 = DAResBlock3d(64, 64, fixed_block=fixed_block)
    self.forw3_2 = DAResBlock3d(64, 64, fixed_block=fixed_block)
    self.forw3_3 = DAResBlock3d(64, 64, fixed_block=fixed_block)

    self.forw4_1 = DAResBlock3d(64, 64, fixed_block=fixed_block)
    self.forw4_2 = DAResBlock3d(64, 64, fixed_block=fixed_block)
    self.forw4_3 = DAResBlock3d(64, 64, fixed_block=fixed_block)

    # skip connection in U-net
    self.back2_1 = DAResBlock3d(128, 128, fixed_block=fixed_block)
    self.back2_2 = DAResBlock3d(128, 128, fixed_block=fixed_block)
    self.back2_3 = DAResBlock3d(128, 128, fixed_block=fixed_block)
    ...
    #   cfg.block_id = 6
    # skip connection in U-net
    self.back3_1 = DAResBlock3d(128, 64, fixed_block=fixed_block)
    self.back3_2 = DAResBlock3d(64, 64, fixed_block=fixed_block)
    self.back3_3 = DAResBlock3d(64, 64, fixed_block=fixed_block)
...
class SGDANoduleNetv4(nn.Module):
  def __init__(self, cfg, mode='train'):
    super(SGDANoduleNetv4, self).__init__()

    self.cfg = cfg
    self.mode = mode
    self.feature_net = DAFeatureNet(cfg, 1, 128)
