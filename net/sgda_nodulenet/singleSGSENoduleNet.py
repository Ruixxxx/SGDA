...
from configs.single_config import config
import copy
...
class SEResBlock3d(nn.Module):
    def __init__(self, n_in, n_out, stride=1, reduction=16):
        ...
        self.se = SGSELayer(n_out, reduction, groups=config['groups'], mode=config['SGSE_mode'])
...
class RcnnHead(nn.Module):
    def __init__(self, cfg, in_channels=128):
        ...
        self.logit = nn.Linear(256, self.num_class * 1) #
...
class singleSGSENoduleNet(nn.Module):
    def __init__(self, cfg, mode='train'):
        super(singleSGSENoduleNet, self).__init__()
        self.cfg = cfg
        self.mode = mode
...
if __name__ == '__main__':
    net = singleSGSENoduleNet(config)
