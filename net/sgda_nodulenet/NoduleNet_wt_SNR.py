...
class ChannelGate_sub(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.global_avgpool(x).view(b, c)
        x = self.fc1(x)
...
class FeatureNet(nn.Module):
    def __init__(self, config, in_channels, out_channels):
        ...
        # SE for selection
        self.style_reid_layer1 = ChannelGate_sub(32)
        self.style_reid_layer2 = ChannelGate_sub(64)
        self.style_reid_layer3 = ChannelGate_sub(64)
        self.style_reid_layer4 = ChannelGate_sub(64)
        self.style_reid_layer5 = ChannelGate_sub(64)
    ...
    def forward(self, x):
        ...
        out1_ori = out1
        out_IN_1 = self.IN1(out1)
        out_style_1 = out1 - out_IN_1
        out_style_1_reid_useful = self.style_reid_layer1(out_style_1)
        out1 = out_IN_1 + out_style_1_reid_useful

        out1_pool, _ = self.maxpool2(out1)
        out2 = self.forw2(out1_pool)#64
        #out2 = self.drop(out2)
        out2_ori = out2
        out_IN_2 = self.IN2(out2)
        out_style_2 = out2 - out_IN_2
        out_style_2_reid_useful = self.style_reid_layer2(out_style_2)
        out2 = out_IN_2 + out_style_2_reid_useful

        out2_pool, _ = self.maxpool3(out2)
        out3 = self.forw3(out2_pool)#96
        out3_ori = out3
        out_IN_3 = self.IN3(out3)
        out_style_3 = out3 - out_IN_3
        out_style_3_reid_useful = self.style_reid_layer3(out_style_3)
        out3 = out_IN_3 + out_style_3_reid_useful

        out3_pool, _ = self.maxpool4(out3)
        out4 = self.forw4(out3_pool)#96
        #out4 = self.drop(out4)
        out4_ori = out4
        out_IN_4 = self.IN4(out4)
        out_style_4 = out4 - out_IN_4
        out_style_4_reid_useful = self.style_reid_layer4(out_style_4)
        out4 = out_IN_4 + out_style_4_reid_useful

        rev3 = self.path1(out4)
        comb3 = self.back3(torch.cat((rev3, out3), 1))#96+96
        comb3_ori = comb3
        comb_IN_3 = self.IN5(comb3)
        comb_style_3 = comb3 - comb_IN_3
        comb_style_3_reid_useful = self.style_reid_layer5(comb_style_3)
        comb3 = comb_IN_3 + comb_style_3_reid_useful
...
class NoduleNet_wt_SNR(nn.Module):
    def __init__(self, cfg, mode='train'):
        super(NoduleNet_wt_SNR, self).__init__()
