...
class DAFeatureNet(nn.Module):
  def __init__(self, config, in_channels, out_channels, net_mode):
    ...
    self.preBlock = nn.Sequential(
      ...
      nn.BatchNorm3d(24, momentum=bn_momentum),
      nn.ReLU(inplace=True))
    dpr = [x.item() for x in torch.linspace(0, config['drop_path'], 4)]

    fixed_block = False
    self.forw1_1 = DAResBlock3d_wt_cross_attention_wt_droppath(24, 32, net_mode, drop_path=dpr[0], fixed_block=fixed_block)
    self.forw1_2 = DAResBlock3d_wt_cross_attention_wt_droppath(32, 32, net_mode, drop_path=dpr[1], fixed_block=fixed_block)

    self.forw2_1 = DAResBlock3d_wt_cross_attention_wt_droppath(32, 64, net_mode, drop_path=dpr[2], fixed_block=fixed_block)
    self.forw2_2 = DAResBlock3d_wt_cross_attention_wt_droppath(64, 64, net_mode, drop_path=dpr[3], fixed_block=fixed_block)
