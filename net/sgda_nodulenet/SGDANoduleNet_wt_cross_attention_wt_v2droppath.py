...
class DAResBlock3d(nn.Module):
    def __init__(self, n_in, n_out, net_mode, drop_path, stride=1, fixed_block=False, reduction=16):
        ...
        self.domain_attention = DomainAttention(n_out, reduction=16, nclass_list=cfg['num_class'], fixed_block=fixed_block,
...
class DAFeatureNet(nn.Module):
  def __init__(self, config, in_channels, out_channels, net_mode):
    ...
    self.preBlock = nn.Sequential(
      ...
      nn.BatchNorm3d(24, momentum=bn_momentum),
      nn.ReLU(inplace=True))
    dpr = [x.item() for x in torch.linspace(0, config['drop_path'], 16)]

    fixed_block = False
    self.forw1_1 = DAResBlock3d(24, 32, net_mode, drop_path=dpr[0], fixed_block=fixed_block)
    self.forw1_2 = DAResBlock3d(32, 32, net_mode, drop_path=dpr[1], fixed_block=fixed_block)

    self.forw2_1 = DAResBlock3d(32, 64, net_mode, drop_path=dpr[2], fixed_block=fixed_block)
    self.forw2_2 = DAResBlock3d(64, 64, net_mode, drop_path=dpr[3], fixed_block=fixed_block)

    self.forw3_1 = DAResBlock3d(64, 64, net_mode, drop_path=dpr[4], fixed_block=fixed_block)
    self.forw3_2 = DAResBlock3d(64, 64, net_mode, drop_path=dpr[5], fixed_block=fixed_block)
    self.forw3_3 = DAResBlock3d(64, 64, net_mode, drop_path=dpr[6], fixed_block=fixed_block)

    self.forw4_1 = DAResBlock3d(64, 64, net_mode, drop_path=dpr[7], fixed_block=fixed_block)
    self.forw4_2 = DAResBlock3d(64, 64, net_mode, drop_path=dpr[8], fixed_block=fixed_block)
    self.forw4_3 = DAResBlock3d(64, 64, net_mode, drop_path=dpr[9], fixed_block=fixed_block)

    # skip connection in U-net
    self.back2_1 = DAResBlock3d(128, 128, net_mode, drop_path=dpr[13], fixed_block=fixed_block)
    self.back2_2 = DAResBlock3d(128, 128, net_mode, drop_path=dpr[14], fixed_block=fixed_block)
    self.back2_3 = DAResBlock3d(128, 128, net_mode, drop_path=dpr[15], fixed_block=fixed_block)
      # 64 + 64 + 3, where 3 is the channeld dimension of coord

    # fixed_block = False
    # if cfg.RESNET.FIXED_BLOCKS >= 6:
    #   fixed_block = True
    # else:
    #   cfg.start_fix = True
    #   cfg.layer_index = 0
    #   cfg.block_id = 6
    # skip connection in U-net
    self.back3_1 = DAResBlock3d(128, 64, net_mode, drop_path=dpr[10], fixed_block=fixed_block)
    self.back3_2 = DAResBlock3d(64, 64, net_mode, drop_path=dpr[11], fixed_block=fixed_block)
    self.back3_3 = DAResBlock3d(64, 64, net_mode, drop_path=dpr[12], fixed_block=fixed_block)
...
class SGDANoduleNet_wt_cross_attention_wt_v2droppath(nn.Module):
  def __init__(self, cfg, mode='train'):
    super(SGDANoduleNet_wt_cross_attention_wt_v2droppath, self).__init__()
...
if __name__ == '__main__':
  net = SGDANoduleNet_wt_cross_attention_wt_v2droppath(config)
