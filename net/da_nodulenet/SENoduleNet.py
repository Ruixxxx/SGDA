...
from net.da.se_bank import SELayer
...
class SEResBlock3d(nn.Module):
    def __init__(self, n_in, n_out, stride=1, reduction=16):
        ...
        self.se = SELayer(n_out, reduction)
...
class SEFeatureNet(nn.Module):
    def __init__(self, config, in_channels, out_channels):
        ...
        self.forw1_1 = SEResBlock3d(24, 32) #nn.Sequential(
            # SEResBlock3d(24, 32),
            # SEResBlock3d(32, 32))
        self.forw1_2 = SEResBlock3d(32, 32)

        self.forw2_1 = SEResBlock3d(32, 64)
        self.forw2_2 = SEResBlock3d(64, 64)

        self.forw3_1 = SEResBlock3d(64, 64)
        self.forw3_2 = SEResBlock3d(64, 64)
        self.forw3_3 = SEResBlock3d(64, 64)

        self.forw4_1 = SEResBlock3d(64, 64)
        self.forw4_2 = SEResBlock3d(64, 64)
        self.forw4_3 = SEResBlock3d(64, 64)

        # skip connection in U-net
        self.back2_1 = SEResBlock3d(128, 128)
        self.back2_2 = SEResBlock3d(128, 128)
        self.back2_3 = SEResBlock3d(128, 128)
            # 64 + 64 + 3, where 3 is the channeld dimension of coord

        # skip connection in U-net
        self.back3_1 = SEResBlock3d(128, 64)
        self.back3_2 = SEResBlock3d(64, 64)
        self.back3_3 = SEResBlock3d(64, 64)
...
class SENoduleNet(nn.Module):
    def __init__(self, cfg, mode='train'):
        super(SENoduleNet, self).__init__()

        self.cfg = cfg
        self.mode = mode
        self.feature_net = SEFeatureNet(cfg, 1, 128)
...
if __name__ == '__main__':
    net = SENoduleNet(config)

    input = torch.rand([4, 1, 128, 128, 128])
    input = Variable(input)
    cls_ind = torch.rand(1)
    cls_ind = Variable(cls_ind)
