...
#     areas2 = d2 * h2 * w2
#
#     xx0 = np.maximum(x1 - w1 / 2., np.squeeze(x2 - w2 / 2., axis=-1))
#     yy0 = np.maximum(y1 - h1 / 2., np.squeeze(y2 - h2 / 2., axis=-1))
#     zz0 = np.maximum(z1 - d1 / 2., np.squeeze(z2 - d2 / 2., axis=-1))
#     xx1 = np.minimum(x1 + w1 / 2., np.squeeze(x2 + w2 / 2., axis=-1))
#     yy1 = np.minimum(y1 + h1 / 2., np.squeeze(y2 + h2 / 2., axis=-1))
#     zz1 = np.minimum(z1 + d1 / 2., np.squeeze(z2 + d2 / 2., axis=-1))
#
#     inter_w = np.maximum(0.0, xx1 - xx0)
#     inter_h = np.maximum(0.0, yy1 - yy0)
#     inter_d = np.maximum(0.0, zz1 - zz0)
#     intersect = inter_w * inter_h * inter_d
#     overlap = intersect / (areas1 + areas2 - intersect)
#
#     return overlap.reshape((len(boxes1), len(boxes2)))
...
def detections2mask(detections, masks, img_reso, num_class=28):
    ...
    for i in range(len(detections)):
        ...
        zoomed_crop = zoom(m, 
                    ...
                    order=2)
        [cat - 1][z_start:z_end, y_start:y_end, x_start:x_end] = (zoomed_crop > 0.5).astype(np.uint8)
