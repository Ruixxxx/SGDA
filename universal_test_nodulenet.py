...
from configs.sgda_config import config, train_config, net_config
from net.da_nodulenet.uniNoduleNet import uniNoduleNet
from net.da_nodulenet.SEuniNoduleNet import SEuniNoduleNet
from net.da_nodulenet.SENoduleNet import SENoduleNet
from net.da_nodulenet.DANoduleNet import DANoduleNet
from net.sgda_nodulenet.SGuniNoduleNet import SGuniNoduleNet
from net.sgda_nodulenet.SGSENoduleNet import SGSENoduleNet
from net.sgda_nodulenet.SGDANoduleNetv2 import SGDANoduleNetv2
from net.sgda_nodulenet.SGDANoduleNet_wt_cross_attention_wt_v2droppath import SGDANoduleNet_wt_cross_attention_wt_v2droppath
import time
...
def eval(net, cls_ind, dataset, annotation_dir, data_dir, save_dir=None):
    ...
    for i, (input, truth_bboxes, truth_labels) in tqdm(enumerate(dataset), total=len(dataset), desc='eval'):
        try:
            ...
            # net.set_mode('eval')
            # net.use_rcnn = False
            # input = (torch.randn((1, 1, 128, 128, 128)), 0, torch.randn(1, 1, 6), torch.randn(1, 1),)
            # flops = FlopCountAnalysis(net, input)
            # print(flop_count_table(flops))
            # print(flop_count_str(flops))
            # print("Total", flops.total() / 1e9)
            # exit()
